...
const createStudentFeeValidation = Joi.object({
  ...
  status:             Joi.string().valid("pending", "partial", "paid", "overdue").default("pending"),
...
const updateStudentFeeValidation = Joi.object({
  ...
  // ✅ Add this
  fee_snapshot: Joi.array().items(feeSnapshotItemSchema).min(1),

  // Optional but recommended if snapshot changes
...
// Coaching-compatible: generate fee for a term (body validation)
const generateStudentFeeValidation = Joi.object({
  institute_id: Joi.string()
    .pattern(/^[0-9a-fA-F]{24}$/)
    .required(),
  student_id: Joi.string()
