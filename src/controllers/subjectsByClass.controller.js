...
// Coaching-compatible: get subjects by batch ID
const getSubjectsByBatchId = async (req, res) => {
  try {
    const records = await subjectsByClassService.getSubjectsByBatchId(req.params.batch_id);
    return res.status(statusCode.OK).json({
      success: true,
      isException: false,
      statusCode: statusCode.OK,
      result: records,
      message: 'Subjects by batch retrieved successfully'
    });
  } catch (err) {
    return res.status(err.statusCode || statusCode.INTERNAL_SERVER_ERROR).json({
      success: false,
      ...
      statusCode: err.statusCode || statusCode.INTERNAL_SERVER_ERROR,
      result: null,
      message: err.message || 'Something went wrong while fetching subjects by batch'
    });
  }
};
...
// Coaching-compatible: get subjects by institute + batch
const getSubjectsByInstituteAndBatch = async (req, res) => {
  try {
    const { institute_id, batch_id } = req.params;
    const records = await subjectsByClassService.getSubjectsByInstituteAndBatch(institute_id, batch_id);
    return res.status(statusCode.OK).json({
      success: true,
      isException: false,
      statusCode: statusCode.OK,
      result: records,
      message: 'Subjects by institute and batch retrieved successfully'
    });
  } catch (err) {
    return res.status(err.statusCode || statusCode.INTERNAL_SERVER_ERROR).json({
      success: false,
...
module.exports = {
  ...
  getSubjectsByInstituteAndBatch,
