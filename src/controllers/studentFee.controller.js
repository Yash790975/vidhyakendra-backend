...
const generateStudentFeeForTerm = async (req, res) => {
  try {
    const { error, value } = generateStudentFeeValidation.validate(req.body);
    if (error) {
      return res.status(statusCode.BAD_REQUEST).json({
        success: false,
        isException: false,
    ...
    const studentFee = await studentFeeService.generateStudentFeeForTerm(value);
...
const getStudentFeesByBatchId = async (req, res) => {
  try {
    const filters = {
      academic_year: req.query.academic_year,
      status:        req.query.status,
    ...
    const studentFees = await studentFeeService.getStudentFeesByBatchId(
      req.params.batch_id,
      filters
    );
