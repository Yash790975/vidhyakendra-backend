...
const subjectsByClassSchema = new mongoose.Schema({
  ...
  // Coaching-compatible: links subject assignment to a coaching batch
  batch_id: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'CoachingBatches',
    default: null
...
// Unique compound index — batch_id included so coaching batches can have
// separate subject assignments for the same class/section combination.
subjectsByClassSchema.index(
  { institute_id: 1, class_id: 1, section_id: 1, batch_id: 1, subject_id: 1 },
  { unique: true }
);
...
subjectsByClassSchema.index({ batch_id: 1 });
