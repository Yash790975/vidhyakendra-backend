...
const getSystemLabel = (type) => {
  if (type === 'coaching') return 'Vidhya Kendra Management System';
  return 'Vidhya Kendra Management System';
};
// const getSystemLabel = (type) => {
...
const createStudentAuth = async (data) => { 
  ...
  const student = await Student.findById(data.student_id).populate(
    'institute_id',
  ...
  const instituteType = student.institute_id?.institute_type;
  const systemLabel = getSystemLabel(instituteType);
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #333;">Welcome to the ${systemLabel} Student Portal</h2>
  ...
  await sendEmail(primaryEmail, emailSubject, emailHtml, instituteType);
...
const requestOTP = async (username) => {
  const studentAuth = await StudentAuth.findOne({ username }).populate({
    path: 'student_id',
    populate: {
      path: 'institute_id',
  ...
  const instituteType = studentAuth.student_id?.institute_id?.institute_type;
  const systemLabel = getSystemLabel(instituteType);
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>You have requested to reset your ${systemLabel} Student Portal password. Please use the following OTP to proceed:</p>
...
const changePassword = async (username, old_password, new_password) => {
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Dear ${studentName},</p>
      <p>Your ${systemLabel} Student Portal password has been changed successfully.</p>
...
const resetPassword = async (username, otp, new_password) => {
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Dear ${studentName},</p>
      <p>Your ${systemLabel} Student Portal password has been reset successfully.</p>
...
// // Super-admin panel is always Vidhya Kendra School Management System
// const SYSTEM_LABEL = "Vidhya Kendra School Management System";
...
//     const subject = `Welcome to ${SYSTEM_LABEL} Admin Panel`;
//     const text = `
// Hello ${data.name || ''},

// Your admin account has been created for ${SYSTEM_LABEL} Admin Panel.
...
//     const subject = `Password Reset OTP for ${SYSTEM_LABEL} Admin Panel`;
//     const text = `
// Hello,

// We received a request to reset the password for your ${SYSTEM_LABEL} Admin Panel account.
