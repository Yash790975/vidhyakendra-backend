...
const createSubjectByClass = async (data) => {
  const { institute_id, class_id, section_id, batch_id, subject_name, subject_type, status } = data;
  ...
  try {
    ...
    if (err.code === 11000) {
      const dupErr = new Error(
        'This subject is already assigned to the given class/section/batch for this institute'
      );
...
// Coaching-compatible: get by batch
const getSubjectsByBatchId = async (batchId) =>
  populateRecord(
    SubjectsByClass.find({ batch_id: batchId }).sort({ created_at: -1 })
  );
...
// Coaching-compatible: get by institute + batch
const getSubjectsByInstituteAndBatch = async (instituteId, batchId) =>
  populateRecord(
    SubjectsByClass.find({ institute_id: instituteId, batch_id: batchId }).sort({ created_at: -1 })
  );
