...
const sendEmail = async (to, subject, html) => {
  ...
  const mailOptions = {
    from: `"${getSystemLabel()}" <${process.env.EMAIL_USER}>`,
    to,
    subject,
    html
  };
...
const createAdmin = async (data) => {
  ...
  const systemLabel = getSystemLabel(institute.institute_type);
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Dear ${data.name},</p>
      <p>Your Admin Panel credentials have been created successfully on ${systemLabel}.</p>
...
const verifyLogin = async (email, password, login_type) => {
  ...
  console.log(`Found ${admins.length} admin(s) with email: ${email}`);
  ...
  // ✅ Validate admin_type against institute_type and login_type
  const instituteType = matchedAdmin.institute_id.institute_type; // 'school' | 'coaching' | 'both'
  const adminType = matchedAdmin.admin_type;                       // 'school' | 'coaching' | null

  if (login_type) {
    // If institute is 'both', any admin_type can login to either portal
    if (instituteType !== 'both') {
      // Institute is strictly 'school' or 'coaching'
      if (adminType && adminType !== login_type) {
        const error = new Error(`A ${adminType} admin cannot log in to the ${login_type} portal`);
        error.statusCode = statusCode.FORBIDDEN;
        throw error;
      }
      if (instituteType !== login_type) {
        const error = new Error(`This institute is not registered as a ${login_type} institute`);
        error.statusCode = statusCode.FORBIDDEN;
        throw error;
      }
    }
    // instituteType === 'both' → allow login to any portal, no restriction
  }
...
const requestOTP = async (email, portal_type) => {
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Dear ${admin.name},</p>
      <p>Your OTP for resetting your ${systemLabel} Admin Panel password is:</p>
      <div style="text-align:center; font-size:24px; letter-spacing:5px; background-color:#f5f5f5; padding:20px; border-radius:5px; margin:20px 0;">${otp}</div>
...
const changePassword = async (email, old_password, new_password) => {
  ...
  const systemLabel = getSystemLabel(admin.institute_id?.institute_type);
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Dear ${admin.name},</p>
      <p>Your ${systemLabel} Admin Panel password has been changed successfully.</p>
...
const resetPassword = async (email, otp, new_password) => {
  ...
  const emailHtml = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>Dear ${admin.name},</p>
      <p>Your ${systemLabel} Admin Panel password has been reset successfully.</p>
...
//   // ✅ NEW: Validate admin_type against institute_type and login_type
//   const instituteType = matchedAdmin.institute_id.institute_type; // 'school' | 'coaching' | 'both'
//   const adminType = matchedAdmin.admin_type;                       // 'school' | 'coaching' | null
