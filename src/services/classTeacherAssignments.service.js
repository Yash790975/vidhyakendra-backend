...
// Roles that require a subject_id
const SUBJECT_ROLES = ["subject_teacher", "faculty"];

const createAssignment = async (assignmentData) => {
  // Validation: subject-based roles must have subject_id
  if (SUBJECT_ROLES.includes(assignmentData.role) && !assignmentData.subject_id) {
    throw new CustomError(
      `subject_id is required for ${assignmentData.role} role`,
      statusCode.BAD_REQUEST
    );
  }

  // Validation: batch_incharge must have batch_id           
  if (assignmentData.role === "batch_incharge" && !assignmentData.batch_id) {
    throw new CustomError(
      "batch_id is required for batch_incharge role",
      statusCode.BAD_REQUEST
    );
  }
  ...
  const existingAssignment = await ClassTeacherAssignments.findOne({
    ...
    batch_id: assignmentData.batch_id || null,               
...
const getAllAssignments = async (filters = {}) => {
  ...
  return await ClassTeacherAssignments.find(query)
...
// : Get batch incharge(s) for a specific batch
const getBatchInchargeByBatchId = async (batchId, academicYear = null) => {
  const query = { batch_id: batchId, role: "batch_incharge", status: "active" };
  if (academicYear) query.academic_year = academicYear;
...
const updateAssignment = async (assignmentId, updateData) => {
  ...
  const effectiveRole = updateData.role || assignment.role;

  // subject_id required for subject-based roles
  if (
    SUBJECT_ROLES.includes(effectiveRole) &&
    !updateData.subject_id &&
    !assignment.subject_id
  ) {
    throw new CustomError(
      `subject_id is required for ${effectiveRole} role`,
      statusCode.BAD_REQUEST
    );
  }

  // batch_id required for batch_incharge                    
  if (
    effectiveRole === "batch_incharge" &&
    !updateData.batch_id &&
    !assignment.batch_id
  ) {
    throw new CustomError(
  ...
  Object.keys(updateData).forEach((key) => {
    if (updateData[key] !== undefined) assignment[key] = updateData[key];
  });
...
const endAssignment = async (assignmentId, endDate = null) => {
  ...
  if (assignment.status === "inactive") {
    throw new CustomError("Assignment is already inactive", statusCode.BAD_REQUEST);
  }
...
const getClassTeacherByClassId = async (classId, sectionId = null, academicYear = null) => {
  const query = { class_id: classId, role: "class_teacher", status: "active" };
  if (academicYear) query.academic_year = academicYear;
