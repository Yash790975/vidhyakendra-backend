...
// Helper: format a single attempt document
...
const submitAttempt = async (attemptId) => {
  ...
  // Auto-calculate time taken from started_at → submitted_at (in seconds)
  ...
  const percentage =
    totalMarks > 0 ? parseFloat(((marksObtained / totalMarks) * 100).toFixed(2)) : 0;
  const isPass =
  ...
  const populated = await AssessmentAttempts.findById(attemptId)
  ...
  return formatAttempt(populated);
};
...
const getAttemptsByStudent = async (studentId, filters = {}) => {
  ...
  const attempts = await AssessmentAttempts.find(query)
    .populate("institute_id", "institute_name institute_code")
    .populate("assessment_id", "title total_marks pass_marks show_result_immediately duration_minutes")
    .populate("class_id", "class_name")
    .populate("section_id", "section_name")
    .populate("batch_id", "batch_name")
    .populate("evaluated_by", "full_name teacher_code")
