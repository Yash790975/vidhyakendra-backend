...
const { formatAnswerFields } = require("../utils/dateFormatter");

// Helper: format a single answer document
const formatAnswer = (doc) => {
  const obj = doc.toObject ? doc.toObject() : doc;
  return formatAnswerFields(obj);
};
...
const getAnswersByAttempt = async (attemptId) => {
  const answers = await AssessmentAnswers.find({ attempt_id: attemptId })
    ...
    .populate("attempt_id", "attempt_number status started_at submitted_at")
    .populate("assessment_id", "title assessment_type total_marks")
    .populate(
      "question_id",
      "question_text question_type options correct_options correct_answer_text marks explanation hint order"
    )
