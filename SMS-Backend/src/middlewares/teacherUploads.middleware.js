...
const teacherPhotoStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = path.join(UPLOADS_ROOT, "teacher_photos");
    ensureDir(uploadPath);
    cb(null, uploadPath);
  },
  filename: (req, file, cb) => {
    const teacherName = req.body.full_name || "teacher";
    const filename = generateUniqueFilename(teacherName, "photo", file.originalname);
    cb(null, filename);
  },
});
...
const teacherPhotoFilter = (req, file, cb) => {
  ...
  if (!allowedExt.includes(ext)) {
    return cb(
      new Error("Only .jpg, .jpeg, .png, .webp, and .gif image formats are allowed!"),
      false
    );
  }
  cb(null, true);
